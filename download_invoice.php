...
if (isset($_POST['order_id'])) { // Periksa apakah order_id dikirimkan melalui POST
    $order_id = $_POST['order_id']; // Dapatkan order_id dari POST
} else {
    $order_id = '';
}

// Mendapatkan data order dari database
if ($user_id != '' && $order_id != '') { // Periksa apakah user_id dan order_id ada
    $order_query = mysqli_query($conn, "SELECT * FROM `tb_orders` WHERE user_id = '$user_id' AND order_id = '$order_id'") or die('Query failed'); // Tambahkan kondisi WHERE untuk memfilter berdasarkan user_id dan order_id
...
if ($fetch_orders != null) {
    ...
    $pdf->setHeaderData('image/original.png', 10, 'Invoice - ' . $order_id, 'BOOKSHELF', array(0, 64, 255), array(0, 64, 128)); // Menggunakan $order_id
    ...
    $pdf->Output('invoice-Bookshelf_'.$fetch_orders['order_id'].'.pdf', 'D');
} else {
